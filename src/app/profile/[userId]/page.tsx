        ...
        <CardHeader className="bg-gradient-to-r from-secondary to-muted p-8">
          <div className="flex flex-col items-center text-center space-y-3">
            <Avatar className="w-32 h-32 border-4 border-background shadow-lg">
              <AvatarImage src={profileUser.avatarUrl} alt={profileUser.name} data-ai-hint="abstract person" />
              <AvatarFallback>{profileUser.name.substring(0,2).toUpperCase()}</AvatarFallback>
            </Avatar>
            <CardTitle className="text-3xl font-bold text-primary">{profileUser.name}</CardTitle>
            <CardDescription className="text-muted-foreground">{profileUser.email}</CardDescription>
            {profileUser.bio && <p className="text-sm text-foreground mt-2">{profileUser.bio}</p>}
          </div>
        </CardHeader>
